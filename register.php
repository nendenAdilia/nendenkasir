...
if (isset($_SESSION['admin']) && $_SESSION['admin']['status'] == 1) {
    ...
    if (isset($_POST['register'])) {
        ...
        $status = strip_tags($_POST['id_member']); // Ubah id_member menjadi status

        // Insert user data into the login table
        $insertLoginSql = 'INSERT INTO member (user, pass, status) VALUES (?, md5(?), ?)';
        $insertLoginStmt = $config->prepare($insertLoginSql);
        $insertLoginStmt->execute([$user, $pass, $status]);
